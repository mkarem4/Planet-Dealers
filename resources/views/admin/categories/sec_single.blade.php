...
@section('content')
    ...
    <div class="page-content-wrap">
        <div class="row">
            <div class="col-md-12">
                <form class="form-horizontal" id="jvalidate" method="post" @if(isset($edit)) action="/admin/category/update/sec" @else action="/admin/category/store/sec" @endif>
                    {{csrf_field()}}
                                    ...
                                    <select class="form-control select" name="parent_id">
                                        ...
                                        @foreach($categories as $category)
                                            ...
                                            @forelse($category->subs as $sub)
                                                <option style="text-indent: 10px;" value="{{$sub->id}}" @if(isset($edit) && $edit->parent_id == $sub->id) selected @elseif(old('parent_id') == $sub->id) selected @endif>{{$sub->name}}</option>
                                            @empty
