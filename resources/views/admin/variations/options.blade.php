...
@section('content')
    <ul class="breadcrumb">
        ...
        <li><a>{{$variation->name}}</a></li>
        <li class="active"><a>{{word('options')}}</a></li>
        @if(count($inputs))
                ...
                <div class="col-md-3">
                    <div class="widget widget-success widget-item-icon">
                        ...
                        <div class="widget-data" >
                            ...
                            <div class="widget-title">{{word('active_variations_options')}}</div>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="widget widget-primary widget-item-icon">
                        ...
                        <div class="widget-data">
                            ...
                            <div class="widget-title">{{word('suspended_variations_options')}}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-12 col-xs-12">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading heading-{{lang()}}">
                        <a><button class="btn btn-info" data-toggle="modal" id="btn-create" data-target="#create_variation">{{word('create_variation_option')}}</button></a>
                    </div>
                            ...
                            <table class="table table-striped">
                                ...
                                @forelse($options as $option)
                                    <tr>
                                        ...
                                        <td>{{lang() == 'ar' ? $option->ar_name : $option->en_name}}</td>
                                        <td><span class="label {{$option->status == 'active' ? 'label-success' : 'label-primary'}} label-form">{{word($option->status)}}</span></td>
                                        <td>
                                            <a><button class="btn btn-condensed btn-warning btn-edit" id="{{$option->id}}_edit" data-toggle="modal" data-target="#edit_variation" data-model="{{$option}}" title="{{word('edit')}}"><i class="fa fa-edit"></i></button></a>
                                            @if($option->status == 'active')
                                                <button class="btn btn-primary btn-condensed mb-control" onclick="modal_suspend({{$option->id}})" data-box="#message-box-primary" title="{{word('suspend')}}"><i class="fa fa-minus-circle"></i></button>
                                            @else
                                                <button class="btn btn-success btn-condensed mb-control" onclick="modal_activate({{$option->id}})" data-box="#message-box-success" title="{{word('activate')}}"><i class="fa fa-check-square"></i></button>
                                            @endif

                                            <button class="btn btn-danger btn-condensed mb-control" onclick="modal_destroy({{$option->id}})" data-box="#message-box-danger" title="{{word('delete')}}"><i class="fa fa-trash-o"></i></button>
                                        </td>
                                    </tr>
                                @empty
    ...
    <div class="modal" id="create_variation" tabindex="-1" role="dialog" aria-labelledby="defModalHead" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal jvalidate" method="post" action="/admin/variation/option/store">
                                ...
                                <div class="modal-body {{lang() == 'ar' ? 'switch_en' : 'switch_ar'}}">
                                        ...
                                        <div class="col-md-9 col-xs-12">
                                            <select class="form-control select" name="parent_id_option">
                                                ...
                                                @foreach($all_variations as $var)
                                                    <option value="{{$var->id}}" {{$var->id == $variation->id ? 'selected' : ''}}>{{$var->name}}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group {{ $errors->has('status_option') ? ' has-error' : '' }}">
                                        ...
                                        <div class="col-md-6 col-xs-12">
                                            ...
                                            @include('error', ['input' => 'status_option'])
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="modal" id="edit_variation" tabindex="-1" role="dialog" aria-labelledby="defModalHead" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal" id="edit_jvalidate" method="post" action="/admin/variation/option/update">
                    <div class="modal-body {{lang() == 'ar' ? 'switch_en' : 'switch_ar'}}">
                        <div class="row">
                            <div class="col-md-12" style="text-align: {{lang() == 'ar' ? 'right' : 'left'}};">
                                {{csrf_field()}}
                                <div class="modal-body {{lang() == 'ar' ? 'switch_en' : 'switch_ar'}}">
                                    <div class="form-group {{ $errors->has('parent_id') ? ' has-error' : '' }}">
                                        <label class="col-md-3 col-xs-12 control-label">{{word('variation')}}</label>
                                        <div class="col-md-9 col-xs-12">
                                            <select class="form-control select" name="edit_parent_id">
                                                <option disabled selected>{{word('choose_from_below')}}</option>
                                                @foreach($all_variations as $var)
                                                    <option value="{{$var->id}}" {{$var->id == $variation->id ? 'selected' : ''}}>{{$var->name}}</option>
                                                @endforeach
        ...
        <div class="mb-container">
            <div class="mb-middle warning-msg alert-msg">
                ...
                <div class="mb-content">
                    <p>{{word('activate_msg')}}</p>
                </div>
                <div class="mb-footer buttons">
                    <form method="post" action="/admin/variation/option/change_status" class="buttons">
                        {{csrf_field()}}
    ...
    <div class="message-box message-box-danger animated fadeIn" data-sound="alert" id="message-box-danger">
        <div class="mb-container">
            <div class="mb-middle warning-msg alert-msg">
                ...
                <div class="mb-footer buttons">
                    <form method="post" action="/admin/variation/option/delete" class="buttons">
                        {{csrf_field()}}
    ...
    <script>
        ...
        @if($errors->has('edit_parent_id') || $errors->has('edit_en_name'))
            $('#{{old('id')}}_edit').click();
        @endif
    </script>
@endsection
