...
@section('content')
    <ul class="breadcrumb">
        ...
        <li>{{word('settings')}}</li>
        <li class="active">{{word('sliders')}}</li>
        @if(count($inputs))
                    ...
                    <div class="col-md-3">
                        <div class="widget widget-success widget-item-icon">
                            ...
                            <div class="widget-data" >
                                ...
                                <div class="widget-title">{{word('active_sliders')}}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="widget widget-primary widget-item-icon">
                            ...
                            <div class="widget-data">
                                ...
                                <div class="widget-title">{{word('suspended_sliders')}}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="widget widget-danger widget-item-icon">
                            ...
                            <div class="widget-data">
                                ...
                                <div class="widget-title">{{word('expired_sliders')}}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-12 col-xs-12">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading heading-{{lang()}}">
                        <a href="/admin/settings/slide/create">
                            <button type="button" class="btn btn-info">{{word('create_slide')}}</button>
                        </a>
                    </div>
                            ...
                            <table class="table table-striped">
                                ...
                                @forelse($slides as $slide)
                                    <tr>
                                        ...
                                        <td><span class="label label-form label-info">{{$slide->url}}</span><br/></td>
                                        <td>
                                            <div class="gallery">
                                                <a class="gallery-item-{{lang() == 'ar' ? 'right' : 'left'}}" href="{{$slide->image}}" title="{{$slide->image}}" data-gallery style="padding : 10px 0px 0px 0px;">
                                                    <div class="image">
                                                        <img class="table-image" src="{{$slide->image}}" alt="{{$slide->image}}"/>
                                                    </div>
                                                </a>
                                            </div>
                                        </td>
                                        <td><span class="label {{$slide->expire_at >= \Carbon\Carbon::today()->toDateString() ? 'label-success' : 'label-danger'}} label-form">{{$slide->expire_at}}</span></td>
                                        <td><span class="label {{$slide->status == 'active' ? 'label-success' : 'label-primary'}} label-form">{{word($slide->status)}}</span></td>
                                        <td>
                                            <a href="/admin/settings/slide/{{$slide->id}}/edit"><button class="btn btn-condensed btn-warning" title="{{word('edit')}}"><i class="fa fa-edit"></i></button></a>
                                            @if($slide->status == 'active')
                                                <button class="btn btn-primary btn-condensed mb-control" onclick="modal_suspend({{$slide->id}})" data-box="#message-box-primary" title="{{word('suspend')}}"><i class="fa fa-minus-circle"></i></button>
                                            @else
                                                <button class="btn btn-success btn-condensed mb-control" onclick="modal_activate({{$slide->id}})" data-box="#message-box-success" title="{{word('activate')}}"><i class="fa fa-check-square"></i></button>
                                            @endif
                                            <button class="btn btn-danger btn-condensed mb-control" onclick="modal_destroy({{$slide->id}})" data-box="#message-box-danger" title="{{word('delete')}}"><i class="fa fa-trash-o"></i></button>
                                        </td>
                                    </tr>
                                @empty
            ...
            <div class="mb-container">
                <div class="mb-middle warning-msg alert-msg">
                    ...
                    <div class="mb-content">
                        <p>{{word('activate_slide')}}</p>
                    </div>
                    <div class="mb-footer buttons">
                        <form method="post" action="/admin/settings/slide/change_status" class="buttons">
                            {{csrf_field()}}
    ...
    <div class="message-box message-box-danger animated fadeIn" data-sound="alert" id="message-box-danger">
        <div class="mb-container">
            <div class="mb-middle warning-msg alert-msg">
                ...
                <div class="mb-content">
                    <p>{{word('delete_slide')}}</p>
                </div>
                <div class="mb-footer buttons">
                    <form method="post" action="/admin/settings/slide/delete" class="buttons">
                        {{csrf_field()}}
