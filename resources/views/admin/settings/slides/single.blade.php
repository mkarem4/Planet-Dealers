...
@section('content')
    <ul class="breadcrumb">
        ...
        <li>{{word('settings')}}</li>
        <li><a href="/admin/settings/sliders/index">{{word('slides')}}</a></li>
        @if(isset($edit))
    ...
    <div class="page-content-wrap">
        <div class="row">
            <div class="col-md-12">
                <form class="form-horizontal" id="jvalidate" method="post" @if(isset($edit)) action="/admin/settings/slide/update" @else action="/admin/settings/slide/store" @endif enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group {{ $errors->has('image_mobile') ? ' has-error' : '' }}">
                                <label class="col-md-3 col-xs-12 control-label">{{word('image_mobile')}}</label>
                                <div class="col-md-6 col-xs-12">
                                    <button type="button" class="btn btn-info btn-file">{{word('browse')}}</button>
                                    @if(isset($edit))
                                        <br/><span class="label label-warning" style="padding: 2px;"> {{word('leave_it')}} </span>
                                        <br/>
                                        <div class="gallery">
                                            <a class="gallery-item-{{lang() == 'ar' ? 'right' : 'left'}}" href="{{$edit->image_mobile}}" title="{{$edit->image_mobile}}" data-gallery style="padding : 10px 0px 0px 0px;">
                                                <div class="image">
                                                    <img class="table-image" src="{{$edit->image_mobile}}" alt="{{$edit->image_mobile}}"/>
                                                </div>
                                            </a>
                                        </div>
                                    @endif
                                    <input type="file" name="image_mobile" class="input-file"/>
                                    @include('error', ['input' => 'image_mobile'])
                                </div>
                            </div>
                            @if(isset($edit))
